...
employee.list <- c(paste("Employee", 1:100))
...
salaries.2015 <- runif(100, min=-5000, max=6000) + salaries.2014

# Create a data.frame 'salaries' by combining the 3 vectors you just made
# Remember to set `stringsAsFactors=FALSE`!
salaries <- data.frame(employee.list,salaries.2014,salaries.2015, stringsAsFactors = FALSE)
...
salary.2015.employee.5 <- salaries[57,"salaries.2015"]

# How many employees got a raise?

how.many.got.raise <- length(salaries$got.raise[salaries$got.raise == TRUE])

# What was the value of the highest raise?

highest.raise <- max(salaries$got.raise)

# What was the "name" of the employee who received the highest raise?

name.of.highest.raise <- salaries[salaries$raise == max(salaries$raise),"Employees"]
...
name.of.highest.decrease <- salaries[salaries$raise ==min(salaries$raise), "Employees"]
